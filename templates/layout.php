<?php

use App\Config\Env;
use App\Service\AuthService;
use App\Support\Csrf;
use App\Support\Flash;
?>
<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="Xbox Shop: educational project built with PHP, PDO and MariaDB.">
    <title><?= e($title ?? 'Xbox Shop') ?> · Xbox Shop</title>
    <link rel="stylesheet" href="<?= e(url('assets/app.css')) ?>">
    <script src="<?= e(url('assets/app.js')) ?>" defer></script>
</head>
<body>
<a class="skip-link" href="#contenido">Skip to content</a>
<div class="screen-noise" aria-hidden="true"></div>
<header class="site-header">
    <a class="brand" href="<?= e(url('catalogo')) ?>" aria-label="Xbox Shop, home">
        <span class="brand-mark" aria-hidden="true">X</span>
        <span>XBOX<span class="brand-slash">//</span>SHOP</span>
    </a>
    <nav aria-label="Main navigation">
        <a href="<?= e(url('catalogo')) ?>">Catalogue</a>
        <a href="<?= e(url('api/productos')) ?>">API</a>
        <?php if (AuthService::check()): ?>
            <a href="<?= e(url('admin')) ?>">Admin</a>
            <form action="<?= e(url('admin/logout')) ?>" method="post" class="inline-form">
                <input type="hidden" name="_csrf" value="<?= e(Csrf::token()) ?>">
                <button class="link-button" type="submit">Log out</button>
            </form>
        <?php else: ?>
            <a href="<?= e(url('admin/login')) ?>">Login</a>
        <?php endif; ?>
    </nav>
</header>

<main id="contenido" class="container">
    <?php if (Env::bool('DEMO_MODE', true)): ?>
        <p class="demo-notice"><strong>Educational demo:</strong> purchases are simulated and data is restored periodically.</p>
    <?php endif; ?>
    <div class="flash-region" aria-live="polite">
        <?php foreach (Flash::consume() as $flash): ?>
            <p class="flash flash--<?= e($flash['type']) ?>"><?= e($flash['message']) ?></p>
        <?php endforeach; ?>
    </div>
    <?php require $templateFile; ?>
</main>

<footer class="site-footer">
    <p>PORTFOLIO_BUILD 2026 · PHP 8 · PDO · MARIADB</p>
    <p>Unofficial fan project, not affiliated with Microsoft or Xbox.</p>
</footer>
</body>
</html>
